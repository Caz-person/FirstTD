...
class PlaySceneOld2: SKScene, GADInterstitialDelegate {
    let startWaveButton = SKSpriteNode(imageNamed: "arrow")
    let bombSideBarTrap  = Trap(imageNamed: "bombTrap")
    let bombSideBarTrapSelected = Trap(imageNamed: "bombTrapSelected")
    ...
    let basicSideBarTrap = Trap(imageNamed: "basicTrap")
       ...
       /* Called when a touch begins */
        ...
        for touch in touches {
            ...
            if self.atPoint(location) != trapSideBar {
                if let trapNode = gameGrid.node(atGridPosition: coordinateFromPoint(location)) {
...
//                    
                    ...
                    let windTurbine = createTrap(windTurbineSideBarTrap, trapID: "windTurbine", positionOfTrap: trapPosition, canBeOnPath: false, cost: 50, attackDamage: 0, upgradeName: "energyGenerated", upgradeMultiplier: 0.2, upgradeArray: [windTurbineSideBarTrap.texture!])
...
//        }
    ...
    func createWave(_ typeOfEnemy: String){
        ...
        var enemyPath = enemyGrid.node(atGridPosition: pathPositions[map.startIndex])!.findPath(to: (enemyGrid.node(atGridPosition: pathPositions[0]))!) as! [GKGridGraphNode]
